...
int main(int argc, char* argv[]) {
    ...
    if (!maze.loadFromFile(argv[1])) {
        cerr << "Failed to open map" << argv[1] << "\n";
        return -1;
    }
...
maze.setCell(stair.x,stair.y,CellType::Floor);
    ...
    mvprintw(0,0, "The map is loading..",
    ...
    int tx2,ty2;
        ...
        const int maxtermina=1000;
                ...
                if(!h2->has_key()&& h2->getX()==key->getX()&& h2->getY()==key->getY())
                  ...
                  if(h1->isTrapped() && h2->has_key()
                      && (abs(h1->getX()- h2->getX())
                    + abs(h1->getY() - h2->getY() ) )==1)
                {
                    h1->rescue();
                       done=true;
                         mvprintw(h+2,0,"S saved G and they lived a happy life.");
                         refresh();
                         napms(10000);
                   }
  ...
  if(h2->isTrapped()&& h1->has_key()
                      && (abs(h2->getX()-h1->getX())
                    + abs(h2->getY()) - h1->getY() ) ==1)
                {
                    h2->rescue();
                       done=true;
                       mvprintw(h+2,0,"G saved S and they lived a happy life.");
                         refresh();
                         napms(10000);
                         break;
                         }
                       ...
                       if(turn==maxtermina && !met){
                           mvprintw(h+2,0,"Out of Time! Game Over!");
                            refresh();
                         napms(10000);
                           done=true;
                           break;
                    ...
                    if(h1->isTrapped() && h2->isTrapped() ){
                         mvprintw(h+2,0,"Both heroes are trapped! Game Over!");
                         refresh();
                         napms(10000);
                           done=true;
                           break;
                    ...
                    if(!met &&
                        !h1->isTrapped() && !h2->isTrapped() &&
                    ((abs(h1->getX()-h2->getX()))+ (abs(h1->getY()-h2->getY())))==1)
                        {
                            met=true;
                            clearing=true;
                            path=maze.findPath({h1->getX(),h1->getY()},stair);
                            mvprintw(h+2,0,"The two heroes found each other! Happy Ending!");
                            refresh();
                            napms(10000);
                       ...
                       if(clearing){
                           if(remI<walls.size()){
                            ...
                            clearing=false;
                            marching=true;
                            maze.setCell(stair.x,stair.y,CellType::Stair);
                            mvprintw(h+2,0,"The heroes are heading to the ladder!");
                            refresh();
                            napms(10000);
                        }
                    }
                            ...
                            mvprintw(h+2,0,"They found the ladder! Happy Ending!");
                            refresh();
                            napms(10000);
                            break;
                        }
